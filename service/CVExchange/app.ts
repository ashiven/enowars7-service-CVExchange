...
app.use((req: types.RequestV2, res: Response, next: NextFunction) => {
...
app.get('/', getuserid, getusername, getuserkarma, getsubids, getsubs, gettopsubs, async (req: types.RequestV2, res: Response) => {
  try {
    const pagelimit = 15
    let page = 1
    let sort = 'hot'
    ...
    if (req.userId && postIds.length > 0) {
      const [ratings] = await req.database.query(ratingsQuery, ratingsParams)
...
app.get('/uploads/:userId/private/:filename', auth, fileAuth, async (req: types.RequestV2, res: Response) => {
  try {
...
app.get('/uploads/:userId/public/:filename', auth, async (req: types.RequestV2, res: Response) => {
  try {
...
app.get('/search', auth, getusername, getuserkarma, getsubids, getsubs, gettopsubs, async (req: types.RequestV2, res: Response) => {
  try {
    const pagelimit = 15
    let page = 1
    if (req.query.page) {
