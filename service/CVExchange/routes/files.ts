...
const publicStorage = multer.diskStorage({
  destination: async (req: types.RequestV2, _file: Express.Multer.File, cb: any) => {
    uploadDest('public', req, cb)
  },
  filename: async (req: types.RequestV2, file: Express.Multer.File, cb: any) => {
...
const fileFilter = async (req: types.RequestV2, file: Express.Multer.File, cb: FileFilterCallback) => {
  try {
...
const fileFilterPrivate = async (req: types.RequestV2, file: Express.Multer.File, cb: FileFilterCallback) => {
  try {
    const regex = /\.(txt)$/i
...
router.get('/retrieve/:userId/:filename', auth, async (req: types.RequestV2, res: Response) => {
  try {
