...
interface map {[key: number]: CommentsV2}

// Route definitions

router.get('/new', auth, getusername, getuserkarma, getsubids, getsubs, gettopsubs, async (req: types.RequestV2, res: Response) => {
  try {
...
router.post('/new', auth, getusername, getsubids, getsubs, async (req: types.RequestV2, res: Response) => {
...
router.get('/:id', auth, getusername, getuserkarma, getsubids, getsubs, gettopsubs, async (req: types.RequestV2, res: Response) => {
  try {
    ...
    for (const comment of rootComments) {
      comment.children.sort((a: types.Comments, b: types.Comments) => b.rating - a.rating)
    }
...
router.get('/edit/:id', auth, getusername, getuserkarma, getsubids, getsubs, gettopsubs, async (req: types.RequestV2, res: Response) => {
  try {
    const postId = req.params.id
    if (!Number.isInteger(parseInt(postId))) {
