...
router.post('/new', auth, getusername, async (req: types.RequestV2, res: Response) => {
...
router.get('/subscribe/:id', auth, getsubids, async (req: types.RequestV2, res: Response) => {
  const connection = await req.database.getConnection()
...
router.get('/search/:id', auth, getusername, getuserkarma, getsubids, getsubs, gettopsubs, async (req: types.RequestV2, res: Response) => {
  try {
    const pagelimit = 15
    let page = 1
    if (req.query.page) {
